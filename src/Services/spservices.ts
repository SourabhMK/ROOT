...
export default class spservices {
   ...
   * @memberof spservices
   */
  public async getUserById(userId: number, siteUrl: string) {
   ...
   * @memberof spservices
   */
  public async getUserByLoginName(loginName: string, siteUrl: string) {
    ...
    try {
      ...
      return  await web.siteUsers.getByLoginName(loginName).get();
      //results = await web.siteUsers.getByLoginName(userId).get();
    } catch (error) {
      return Promise.reject(error);
    }
  ...
  public async getSiteRegionalSettingsTimeZone(siteUrl: string) {
    try {
      const web = Web(siteUrl);
      return await web.regionalSettings.timeZone.usingCaching().get();
    } catch (error) {
      return Promise.reject(error);
    }
  }

  // TODO: DIPAL - JQUERY MIGHT BE REQUIRED FOR CALLING EXTERNAL API - let find other options
  /**
   * @param {string} webUrl
