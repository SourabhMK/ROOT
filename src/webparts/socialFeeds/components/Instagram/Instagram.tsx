...
export default class InstagramFeed extends React.Component<IInstagramProps, { error: IError, isLoaded: Boolean, items: IInstagramFeed }> {
  constructor(props) {
    super(props);
    this.state = {
      error: null,
  ...
  private handleFailure(error) {
    let failure: IError = {
      ...
      message: "Failed to load data, please check WebPart settings or try again later.",
      status: error
    };
  ...
  private getData(count) {
    var params = {
      url: `https://www.instagram.com/${this.props.username ? this.props.username.trim() : "thenikhilk"}/?__a=1`,
      container: 'none'
    };
