...
import { IAssignedToViewSelectProps } from './IAssignedToViewSelectProps';
import { escape } from '@microsoft/sp-lodash-subset';
import { DefaultButton, PrimaryButton, CompoundButton } from '@fluentui/react/lib/Button';
import {IAssignedToViewSelectState} from './IAssignedToViewSelectState';
...
// debugger;
export default class AssignedToViewSelect extends React.Component<IAssignedToViewSelectProps, IAssignedToViewSelectState> {
 ...
 private initialDeptGetMethod() {
  ...
  this.props.spHttpClient
    .get(`${this.props.webUrl}/_api/web/lists/GetByTitle('EmployeeRequest')/items?$select=*,AttachmentFiles&$expand=AttachmentFiles&$filter=ReAssignToId eq ${this.props.currentUserId}&$orderby=ID desc`,
...
private async getInProcessRequestsCount(deptName){
  ...
  return this.props.spHttpClient
    .get(`${this.props.webUrl}/_api/web/lists/GetByTitle('EmployeeRequest')/items?$select=*&$filter=((ReAssignToId eq ${this.props.currentUserId}) and (Status eq 'In Process') and (Department eq '${deptName}'))&$orderby=ID desc`,
    SPHttpClient.configurations.v1, {
      headers: headers
    })
    .then(result=>{
...
private async getclosedRequestsCount(deptName){
  ...
  headers.append("accept", "application/json;odata.metadata=none");
  return this.props.spHttpClient
    .get(`${this.props.webUrl}/_api/web/lists/GetByTitle('EmployeeRequest')/items?$select=*&$filter=((ReAssignToId eq ${this.props.currentUserId}) and (Status eq 'Completed') and (Department eq '${deptName}'))&$orderby=ID desc`,
    SPHttpClient.configurations.v1, {
      headers: headers
    })
    .then(result=>{
...
homeButtonClick(){
  ...
  public render(): React.ReactElement<IAssignedToViewSelectProps> {
    return (
      <div className={styles.assignedToViewSelect}>
         <div>
          {
            this.state.loading &&
      ...
      {(this.state.inProcessRequestsCount === this.state.raisedRequestsDepts.length) && (this.state.closedRequestsCount === this.state.raisedRequestsDepts.length) && (this.state.unlockEachPlaceCard === 0) && (this.state.homeButtonUnlock === 0) &&
       <div className="ms-Grid">
            ...
            
        {
            ...
            <AssignedToView passedDept={this.state.passDeptName} passedStatus={this.state.passStatus} deptBelongingNames={[]} msGraphClientFactory={this.props.msGraphClientFactory} emailType={this.props.emailType} description={this.props.description} loggedInUserEmail={this.props.loggedInUserEmail} loggedInUserName={this.props.loggedInUserName} spHttpClient={this.props.spHttpClient} webUrl={this.props.webUrl} currentUserId={this.props.currentUserId}/>
        }
