...
import styles from './ManagerViewSelect.module.scss';
import { IManagerViewSelectProps } from './IManagerViewSelectProps';
import { escape } from '@microsoft/sp-lodash-subset';
import { DefaultButton, PrimaryButton, CompoundButton } from '@fluentui/react/lib/Button';
import {IManagerViewSelectState} from './IManagerViewSelectState';
...
debugger;
export default class ManagerViewSelect extends React.Component<IManagerViewSelectProps, IManagerViewSelectState> {
 ...
 private initialDeptGetMethod() {
  ...
  this.props.spHttpClient
    .get(`${this.props.webUrl}/_api/web/lists/GetByTitle('EmployeeRequest')/items?$select=*,DepartmentManager/Title,AttachmentFiles&$expand=DepartmentManager,AttachmentFiles&$filter=DepartmentManagerId eq ${this.props.currentUserId} &$orderby=ID desc`,
...
private async getOpenRequestCount(deptName){
  ...
  return this.props.spHttpClient
    .get(`${this.props.webUrl}/_api/web/lists/GetByTitle('EmployeeRequest')/items?$select=*&$filter=((Author eq ${this.props.currentUserId}) and (Status eq 'Pending') and (Department eq '${deptName}'))&$orderby=ID desc`,
    SPHttpClient.configurations.v1, {
      headers: headers
    })
    .then(result=>{
...
private async getInProcessRequestsCount(deptName){
  ...
  headers.append("accept", "application/json;odata.metadata=none");
  return this.props.spHttpClient
    .get(`${this.props.webUrl}/_api/web/lists/GetByTitle('EmployeeRequest')/items?$select=*&$filter=((Author eq ${this.props.currentUserId}) and (Status eq 'In Process') and (Department eq '${deptName}'))&$orderby=ID desc`,
    SPHttpClient.configurations.v1, {
      headers: headers
    })
    .then(result=>{
...
private async getclosedRequestsCount(deptName){
  ...
  headers.append("accept", "application/json;odata.metadata=none");
  return this.props.spHttpClient
    .get(`${this.props.webUrl}/_api/web/lists/GetByTitle('EmployeeRequest')/items?$select=*&$filter=((Author eq ${this.props.currentUserId}) and (Status eq 'Completed') and (Department eq '${deptName}'))&$orderby=ID desc`,
    SPHttpClient.configurations.v1, {
      headers: headers
    })
    .then(result=>{
...
for(i=0;i<SPGroupList.length;++i){
  ...
  var test = {
    ...
    headers.append("accept", "application/json;odata.metadata=none");
    this.props.spHttpClient
      .get(`${this.props.webUrl}/_api/web/lists/getbytitle('EmployeeRequest')/items?$select=*,Author/Title&$expand=Author &$filter=Status eq 'Pending' and AssignedTo eq '${SPGroupList[i].groupname}' &$orderby=ID desc`,
      SPHttpClient.configurations.v1, {
        headers: headers
      })
      .then((res: SPHttpClientResponse): Promise<any> => {
...
homeButtonClick(){
  ...
  public render(): React.ReactElement<IManagerViewSelectProps> {
    return (
      <div className={ styles.managerViewSelect }>
         <div>
          {
            this.state.loading &&
      ...
      { (this.state.openRequestsCount === this.state.raisedRequestsDepts.length) && (this.state.inProcessRequestsCount === this.state.raisedRequestsDepts.length) && (this.state.closedRequestsCount === this.state.raisedRequestsDepts.length) && (this.state.unlockEachPlaceCard === 0) && (this.state.homeButtonUnlock === 0) &&
       <div className="ms-Grid">
            ...
            
        {
            ...
            <ManagerView passedDept={this.state.passDeptName} passedStatus={this.state.passStatus}  msGraphClientFactory={this.props.msGraphClientFactory} emailType={this.props.emailType} description={this.props.description} loggedInUserEmail={this.props.loggedInUserEmail} loggedInUserName={this.props.loggedInUserName} spHttpClient={this.props.spHttpClient} webUrl={this.props.webUrl} currentUserId={this.props.currentUserId}/>
        }
